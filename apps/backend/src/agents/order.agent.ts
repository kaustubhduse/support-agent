import { runTogetherAgent } from "../lib/together_simple";
import { z } from "zod";
import { fetchOrder, cancelOrder } from "../tools/order.tool";

export const orderAgent = async (message: string, conversationId: string) => {
  try{
    const { text } = await runTogetherAgent({
      system: `You are an order management specialist. Help customers with order status, tracking information, modifications, and cancellations. You can cancel orders when customers request it. Be clear and informative about order details.`,
      prompt: message,
      tools: {
        fetchOrder: {
          description: "Fetch order details by order ID",
          parameters: z.object({
            orderId: z.string().describe("The order ID to look up"),
          }),
          execute: async ({ orderId }: any) => {
            const order = await fetchOrder(orderId);
            return order || { error: "Order not found" };
          },
        },
        cancelOrder: {
          description: "Cancel an order by order ID. Use this when customer requests order cancellation.",
          parameters: z.object({
            orderId: z.string().describe("The order ID to cancel"),
          }),
          execute: async ({ orderId }: any) => {
            const order = await cancelOrder(orderId);
            return order 
              ? { success: true, message: `Order ${orderId} has been cancelled`, order }
              : { error: "Order not found or already cancelled" };
          },
        },
      },
    });

    return text;
  } 
  catch(error){
    console.error("[Order Agent] Error:", error);
    return "I'm currently running in offline mode due to API limits. I can't look up live orders right now, but normally I would help you with that!";
  }
};
